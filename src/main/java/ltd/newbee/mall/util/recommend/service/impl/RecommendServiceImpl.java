package ltd.newbee.mall.util.recommend.service.impl;
...
import ltd.newbee.mall.util.recommend.core.ItemCF;
...
public class RecommendServiceImpl implements RecommendService {
    ...
    public List<RelateDTO> getRelateData() {
        ...
        List<OrderItemVO> newBeeMallOrderItems = new ArrayList<>();
        if (orderIds.size() != 0) {
             newBeeMallOrderItems = orderItemDao.selectByOrderIds(orderIds);
        }
    ...
    public List<Goods> recommendGoods(Long userId, Integer num) {
        ...
        if (recommendGoods.size() == 0) {
            List<Goods> goods = this.goodsDao.selectUpGoodsList();

            if (goods.size() > 10) {
                Collections.shuffle(goods);
                return goods.subList(0, 10);
            }else{
                return goods;
            }
        }
